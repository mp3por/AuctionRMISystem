...
public class Auction extends UnicastRemoteObject implements IAuctionRemote {
    ...
    /**
     * Can be used to see all active items in the version1.velin.server.Auction
    ...
    /**
     * Creates and Registers an version1.velin.server.Auction Item to this version1.velin.server.Auction
     *
    ...
    public long createAndRegisterAuctionItem(long creatorId, String itemName, double value, Date closingDate) throws RemoteException, AuctionException {
        try {
            ...
            System.out.format("AUCTION: Client (-- %d --) created and registered a new version1.velin.server.AuctionItem {%d,%s,%f,%s}.\n", creatorId, newAuctionItemId, itemName, value, formatter.format(closingDate));
            return newAuctionItemId;
        } catch (AuctionItem.AuctionItemNegativeStartValueException e) {
            System.out.format("AUCTION: Client (%d) wanted to create invalid version1.velin.server.AuctionItem. Error:%s.\n", creatorId, e.getShortName());
    ...
    /**
     * Registers an version1.velin.server.Auction Item to this version1.velin.server.Auction
     *
    ...
    public boolean bidForItem(long bidderId, long itemId, double bidValue) throws RemoteException, AuctionException {
        ...
        if (item != null) {
            ...
            throw new AuctionException(String.format("This item is no longer available for bidding or there is no version1.velin.server.Auction Item with id '%d'.", itemId));
        }
    }

    /**
     * Register as a participant in this version1.velin.server.Auction
     *
    ...
    /**
     * Clients can use this method to unregister from this version1.velin.server.Auction. This way they will not receive notifications.
     *
    ...
    /**
     * Method to call when a version1.velin.server.AuctionItem is complete. It logs the data, removes the item from the list with live
     * version1.velin.server.Auction Items and notifies all clients with the result.
     *
     * @param finishedAuctionItem
     */
    public void itemCompleteCallback(IAuctionItem finishedAuctionItem, List<Long> bidders) {
        System.out.format("AUCTION: version1.velin.server.Auction notified that item {ID: %d,name: %s,finalValue: %f, lastBidder:  %d} has finished.\n",
        ...
        for (long key : bidders) {
            try {
                ...
                System.out.format("AUCTION: Client (-- %d --) notified of finished version1.velin.server.AuctionItem '%s'.\n", key, finishedAuctionItem.getItemName());
    ...
    /**
     * A wrapper for finished version1.velin.server.Auction Items so that they unregister themselves after a specific amount of time
     */
    private static class FinishedAuctionItem {
        ...
        private void removeFromReady() {
            ...
            System.out.println("AUCTION: version1.velin.server.AuctionItem (" + auctionItemId + ") expired and can not be reclaimed anymore.");
        }
    }
}
